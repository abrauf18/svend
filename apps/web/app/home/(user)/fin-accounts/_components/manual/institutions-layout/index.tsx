...
export default function InstitutionsLayout() {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      const res = await fetch('/api/fin-account-mgmt/manual/csv', {
        method: 'POST',
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-full w-full">
        {/* Left side - Institutions - Ajustado para ocupar más espacio */}
        <div className="flex h-full w-full flex-shrink-0 flex-col">
          <div className="flex-1 overflow-y-auto">
            <div
              onScroll={handleScroll}
              className="institutions-scroll-container scrollbar-thin scrollbar-track-transparent scrollbar-thumb-muted-foreground/20 hover:scrollbar-thumb-muted-foreground/40 h-[calc(100vh-180px)] overflow-y-auto pr-4 [mask-image:linear-gradient(to_bottom,transparent,black_var(--mask-top,20px),black_calc(100%-20px),transparent)]"
            >
              <div className="flex flex-col gap-2 pb-6">
                {/* Accordion con ancho completo */}
                <Accordion type="single" collapsible defaultValue="manual" className="w-full">
                  <AccordionItem value="manual" className="w-full">
        ...
        {/* Right side - Transactions - Ajustado para ocupar el espacio restante */}
        <div className="h-full flex-1 pl-4 min-w-0">
          <TransactionsRender />
          <TransactionSideMenu />
        </div>
      </div>
