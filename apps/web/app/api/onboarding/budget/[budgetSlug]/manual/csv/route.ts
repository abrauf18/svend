...
// POST /api/onboarding/budget/manual/csv
...
export const POST = enhanceRouteHandler(
  async ({ body, params, user }) => {
    ...
    try {
      ...
      const { isValid, missingProps, mappableProps, invalidRows } =
        checkCSVValidity({
          csv: parsedText,
          csvValidProps,
        });
      ...
      const {
        ...
        budgetAccounts: insertedBudgetAccounts,
      } = data as {
      ...
      const linkedFinAccounts = insertedAccounts
        .map((account) => {
          const institution = insertedInstitutions.find(
            (inst) => inst.id === account.institution_id,
          );
          if (!institution) {
            console.error(
              '[CSV Processing] Could not find institution for account:',
              {
                accountId: account.id,
                institutionId: account.institution_id,
                availableInstitutions: insertedInstitutions.map((i) => ({
                  id: i.id,
                  name: i.name,
                })),
              },
          ...
          const budgetAccount = insertedBudgetAccounts.find(
            (ba) => ba.manual_account_id === account.id,
          );
      ...
      if (linkedFinAccounts.length === 0) {
        return NextResponse.json(
          {
            error:
              "No valid accounts could be processed. This might be because the accounts in the CSV file don't match the existing accounts.",
          },
          { status: 400 },
        );
      }
      ...
      const summary = {
        newInstitutions:
          insertedInstitutions.length - (repeatedInstitutions?.size || 0),
      ...
      return NextResponse.json({
        ...
        linkedFinAccounts,
        warning:
          linkedFinAccounts.length < insertedBudgetAccounts.length
