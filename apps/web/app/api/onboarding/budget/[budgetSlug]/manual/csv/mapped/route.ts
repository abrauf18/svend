...
// Utilities
import { CSVProcessingError, normalizeCategory } from '../route';
...
// POST /api/onboarding/budget/manual/csv/mapped
...
export const POST = enhanceRouteHandler(
  async ({ body, params, user }) => {
    ...
    try {
      ...
      const { data: rawCsvData } = papa.parse<Record<string, string>>(text, {
        header: true,
      });
      ...
      const {
        ...
        missingProps,
        mappableProps,
        invalidRows: validityInvalidRows,
        error: csvValidityError,
      } = checkCSVValidity({
      ...
      const unmappedRequiredColumns = (missingProps || []).filter(
        (col) =>
