  ...
  try {
    ...
    const validRows = parsedText.filter((row) => {
      if (!row.BankName?.trim() || !row.BankSymbol?.trim()) {
        console.warn(
          '[Insert Institutions] Skipping row with missing Institution or Symbol:',
          row,
        );
        return false;
      }
    ...
    for (const trans of validRows) {
      ...
      if (nameExists || symbolExists) {
        ...
        const duplicateNameInNew = Array.from(
          nonRepeatedInstitutions.values(),
        ).find(
          (inst) =>
            inst.BankName.trim().toLowerCase() === parsedInstitutionName,
        );

        const duplicateSymbolInNew = Array.from(
          nonRepeatedInstitutions.values(),
        ).find((inst) => inst.BankSymbol.trim().toUpperCase() === parsedSymbol);

        if (duplicateNameInNew || duplicateSymbolInNew) {
          console.warn(
            '[Insert Institutions] Found duplicate in new entries:',
            {
              ...
              existing: Array.from(nonRepeatedInstitutions.values()).map(
                (i) => ({
                  name: i.BankName,
                  symbol: i.BankSymbol,
                }),
    ...
    if (nonRepeatedInstitutions.size > 0) {
      ...
      const allInstitutions = [
        ...data,
        ...Array.from(repeatedInstitutions.values()),
      ];
      ...
      return {
        data: Array.from(
          new Map(
            Array.from(repeatedInstitutions.values()).map((inst) => [
              inst.id,
              inst,
            ]),
          ).values(),
