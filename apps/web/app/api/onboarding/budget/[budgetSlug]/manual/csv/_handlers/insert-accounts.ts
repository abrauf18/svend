...
type BudgetAccount =
  Database['public']['Tables']['budget_fin_accounts']['Insert'];
...
function validateAccountType(type: string): AccountType {
  ...
  console.warn(
    `Invalid account type "${type}" provided, defaulting to "other"`,
  );
  return 'other';
}
...
export default async function insertAccounts({
  ...
  try {
    ...
    const validRows = parsedText.filter(
      (row) =>
        row.AccountName &&
        row.BankName &&
    ...
    for (const trans of validRows) {
      ...
      const matchingInstitution = insertedInstitutions.find(
        (inst) =>
          ...
          (inst.symbol ?? '').trim().toUpperCase() ===
            trans.BankSymbol.trim().toUpperCase(),
      );

      if (!matchingInstitution) {
        console.warn(
          `Skipping account "${parsedAccountName}" - Institution mismatch:`,
          {
            requested: {
            ...
            institution_id: insertedInstitutions.find(
              (inst) =>
                inst.name.trim().toLowerCase() ===
                  acc.BankName.trim().toLowerCase() &&
                inst.symbol.trim().toUpperCase() ===
                  acc.BankSymbol.trim().toUpperCase(),
            )?.id,
