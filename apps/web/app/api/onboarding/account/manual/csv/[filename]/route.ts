    ...
    try {
      ...
      if (institutionsError) {
        console.error('[CSV Processing] Failed to process institutions:', {
          ...
          details: typeof institutionsError === 'object' ? institutionsError : undefined,
          stack: institutionsError instanceof Error ? institutionsError.stack : undefined
        });
        return NextResponse.json(
      ...
      const insertedTransactions = insertedAccounts.map((acc) => {
        const transactionsRows = parsedText.filter(
          (row) => 
            row.AccountName === acc.name && 
            row.BankName.trim().toLowerCase() === insertedInstitutions.find(
              inst => inst.id === acc.institution_id
            )?.name.trim().toLowerCase() &&
            row.BankSymbol.trim().toUpperCase() === insertedInstitutions.find(
              inst => inst.id === acc.institution_id
      ...
      const uniqueTransactions = insertedTransactions.filter((trans, index, self) =>
        index === self.findIndex(t => t.TransactionId === trans.TransactionId)
      );
      ...
      const { data: existingTransactions, error: existingTransactionsError } = 
        await supabaseAdmin
          ...
          `)
          .in('user_tx_id', uniqueTransactions.map(t => t.TransactionId))
          .eq('manual_fin_accounts.owner_account_id', user.id);

      if (existingTransactionsError) {
        console.error('[CSV Processing] Error checking existing transactions:', existingTransactionsError);
        throw existingTransactionsError;
      }

      // Filter out transactions that already exist
      const newTransactions = uniqueTransactions.filter(
        trans => !existingTransactions?.some(
      ...
      const validTransactions = newTransactions.filter(trans => {
        const matchingCategory = builtInCategories.find(
          (cat) => normalizeCategory(cat.category_name ?? '') === normalizeCategory(trans.Category ?? '')
        );

        if (!matchingCategory) {
          console.warn(`Skipping transaction ${trans.TransactionId}: Invalid category "${trans.Category}". Available categories:`, 
      ...
      if (validTransactions.length > 0) {
        const {
          error: budgetFinAccountTransactionsError,
          ...
          p_transactions: validTransactions.map((trans) => {
            ...
            return {
              ...
              budget_fin_account_id: insertedBudgetAccounts.find(
                (fin_acc) => fin_acc.manual_account_id === insertedAccounts.find(
                  acc => acc.name === trans.AccountName
                )?.id!,
              )?.id!,
        ...
        if (budgetFinAccountTransactionsError) throw budgetFinAccountTransactionsError;
      }
      ...
      const {
        ...
        .from('fin_account_transactions')
        .select('*')
        .in('user_tx_id', validTransactions.map((trans) => trans.TransactionId));
...
const normalizeCategory = (category: string): string => {
  return category.trim().toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ');
}