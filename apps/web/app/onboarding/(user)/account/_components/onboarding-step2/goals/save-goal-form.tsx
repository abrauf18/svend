...
export function SaveGoalForm(props: {
  ...
  const currentGoal = state.account.budget?.goals?.find(
    goal => goal.type === 'savings'
  );
  ...
  useEffect(() => {
    ...
    if (currentGoal) {
      const initialData = {
        ...
        targetDate: currentGoal.targetDate,
        description: currentGoal.description || '',
        subType: currentGoal.subType || 'general'
      };
      ...
      <form className="space-y-6">
        <div className="max-w-[50%]">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Savings Type</FormLabel>
                <Select
                  value={field.value}
                ...
                >
                  ...
                  <SelectContent>
                    {Object.entries(saveSubTypes).map(([key, label]) => (
