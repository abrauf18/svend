...
export default function OnboardingStep2AnalyzingData() {
  const { state, accountBudgetUpdate, accountNextStep, budgetSlug } = useBudgetOnboardingContext();
  ...
  useEffect(() => {
    if (
      state.budget.budget.spendingTracking &&
      Object.keys(state.budget.budget.spendingTracking).length > 0 &&
      state.budget.contextKey === 'analyze_spending'
    ) {
      accountNextStep();
    }
  }, [state.budget.budget.spendingTracking]);
  ...
  const analyzeSpending = async () => {
    const response = await fetch(`/api/onboarding/budget/${budgetSlug}/budget/analysis`, {
      method: 'POST',
      headers: {
    ...
    const handleBudgetUpdate = () => {
      ...
      const updatedGoals = (state.budget.budget.goals ?? []).map(goal => ({
        ...goal,
      ...
      const newBudget: Budget = {
        ...
        linkedFinAccounts: state.budget.budget.linkedFinAccounts
      } as Budget;
