...
export const BudgetTable = forwardRef<HTMLFormElement, BudgetTableProps>((props, ref) => {
    ...
    useEffect(() => {
        async function initializeCategories() {
            const allCategories = await categoryService.getBudgetCategoryGroups(state.budget.budget?.id);
            const budgetSpendingRecommendationsBalanced = state.budget.budget?.spendingRecommendations.balanced || {};
            const budgetSpendingTrackingsByMonth = state.budget.budget?.spendingTracking || {};
            ...
            form.reset({ 
                categoryGroups: sortedSpendings.filter(group => group.groupName !== state.budget.budget?.id) 
            });
        }
