...
export default function TransactionSideMenu() {
  ...
  const selectedTransaction =
    state.budget.transactions?.sideMenu?.selectedTransaction;

  const [isLoading, setIsLoading] = useState(false);

  const transaction = useMemo(() => {
    if (!state.budget.manualInstitutions || !selectedTransaction) return null;

    return state.budget.manualInstitutions
      .flatMap((inst) => inst.accounts)
      .flatMap((acc) => acc.transactions)
      .find((t) => t.id === selectedTransaction);
  }, [state.budget.manualInstitutions, selectedTransaction]);

  const accounts = useMemo(() => {
    if (!state.budget.manualInstitutions) return {};

    const parsedAccounts: Record<string, any[]> = {};

    state.budget.manualInstitutions.forEach((inst) => {
  ...
  useEffect(() => {
    if (transaction) {
      reset({
        ...
        tx_status: (transaction.status || 'POSTED').toUpperCase() as
          | 'PENDING'
          | 'POSTED',
      });
    }
  ...
  async function handleSubmitTransaction(
    ...
    try {
      ...
      const res = await fetch(
        `/api/onboarding/budget/${budgetSlug}/manual/transactions/${transaction?.id}`,
        {
          method: 'PUT',
          body: JSON.stringify({
            ...data,
        ...
        >
          ...
          <div className={`flex flex-col gap-4`}>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setValue('tx_status', value as 'PENDING' | 'POSTED')
              }
            >
              <SelectTrigger>
            ...
            <div className="flex w-full flex-col gap-4 sm:flex-row sm:justify-end">
              ...
              <Button
                ...
                disabled={
                  !formState.isValid ||
                  isLoading ||
                  (transaction?.meta_data?.created_for != null &&
                    state?.budget?.budget?.id != null)
                    ? transaction?.meta_data?.created_for !==
