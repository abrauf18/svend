...
export default function CreateInstitution({ setIsCreatingInstitution }: Props) {
  const { accountManualInstitutionsAddOne, state,budgetSlug } = useBudgetOnboardingContext();
  ...
  function checkIfInstitutionExists(name: string) {
    if (!state.budget.manualInstitutions) return false;

    return state.budget.manualInstitutions.some(
      (institution) =>
  ...
  async function handleCreateInstitution(
    ...
    try {
      ...
      const res = await fetch(`/api/onboarding/budget/${budgetSlug}/manual/institutions`, {
        method: 'POST',
