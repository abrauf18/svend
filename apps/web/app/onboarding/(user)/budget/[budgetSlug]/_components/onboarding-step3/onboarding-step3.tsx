...
function OnboardingStep3CreateBudget() {
  ...
  const { state, budgetSlug } = useBudgetOnboardingContext();

  useEffect(() => {
    function calculateDiscretionarySavings() {
      const budgetSpendingRecommendationsBalanced = state.budget.budget?.spendingRecommendations.balanced || {};
      ...
      const currentSpending = Object.values(budgetSpendingRecommendationsBalanced)
        .reduce((total, group) => {
          ...
          const groupDiscretionarySpending = group.categories
            .filter(category => {
              const categoryConfig = Object.values(state?.budget.svendCategoryGroups || {})
    ...
    function calculateGoalTimeline() {
      const goals = state.budget.budget?.goals || [];
      ...
      const balancedIncome = Math.abs(state.budget.budget?.spendingRecommendations.balanced?.['Income']?.recommendation || 0);
      const balancedSpending = Object.values(state.budget.budget?.spendingRecommendations.balanced || {})
        .filter(group => group.groupName !== 'Income')
        .reduce((total, group) => total + group.recommendation, 0);
      
      const conservativeIncome = Math.abs(state.budget.budget?.spendingRecommendations.conservative?.['Income']?.recommendation || 0);
      const conservativeSpending = Object.values(state.budget.budget?.spendingRecommendations.conservative || {})
        .filter(group => group.groupName !== 'Income')
        .reduce((total, group) => total + group.recommendation, 0);
      
      // Calculate total monthly allocations for all goals
      const balancedGoalAllocations = Object.values(state.budget.budget?.goals || [])
        .reduce((total, goal) => {
      ...
      const conservativeGoalAllocations = Object.values(state.budget.budget?.goals || [])
        .reduce((total, goal) => {
    ...
    if (state.budget.budget?.spendingRecommendations?.balanced) {
      calculateDiscretionarySavings();
      calculateGoalTimeline();
    }
  }, [state.budget.budget?.spendingRecommendations, state.budget.budget?.goals]);
  ...
  const handleBudgetSubmit = async (budgetData: z.infer<typeof BudgetFormSchema>) => {
    ...
    const formReqData = {
      categorySpending: budgetData.categoryGroups
        ...
        .filter(group => group.groupName !== state.budget.budget?.id)
    ...
    try {
      const response = await fetch(`/api/onboarding/budget/${budgetSlug}/budget/spending`, {
        method: 'PUT',
        headers: {
      ...
      window.location.href = `/home/${data.budgetSlug}`;
    } catch (error) {
