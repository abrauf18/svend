...
export default function UpdateAccount({ account, institution, isDisabled }: Props) {
  const { accountManualAccountUpdateOne,budgetSlug } = useBudgetOnboardingContext();
  ...
  async function handleUpdateAccount(data: z.infer<typeof accountSchema>) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/onboarding/budget/${budgetSlug}/manual/accounts/${account.id}`,
        {
          method: 'PUT',
          body: JSON.stringify({
            ...data,
