...
create table if not exists
  public.team_memberships (
    ...
    unique (user_id, team_account_id),
...
create or replace function kit.add_current_user_to_new_team_account() 
returns trigger 
language plpgsql security definer
set search_path = '' as $$
begin
    -- Check if membership already exists to prevent duplicate inserts
    IF EXISTS (
        SELECT 1 
        FROM public.team_memberships 
        WHERE team_account_id = new.id 
        AND user_id = new.primary_owner_user_id
    ) THEN
        raise log 'Team membership already exists for user % and account %', 
            new.primary_owner_user_id, 
            new.id;
        return NEW;
    END IF;

    raise log 'Adding user to team account. User ID: %, Account ID: %, Role: owner', 
        new.primary_owner_user_id, 
        new.id;
    ...
    ON CONFLICT (team_account_id, user_id) DO NOTHING;  -- Handle potential race condition
        
    raise log 'Membership inserted successfully';
    
    return NEW;
exception
    when others then
        raise log 'Error in add_current_user_to_new_team_account: % %', SQLERRM, SQLSTATE;
        return NEW;
end;
$$;
...
-- Verify get_upper_system_role exists and returns a valid role
...
begin
    ...
    if role_name is null then
        raise log 'No role found with hierarchy_level = 1';
        return 'admin'; -- fallback to admin if no role found
    end if;

    raise log 'Upper system role found: %', role_name;
    return role_name;
end;
$$ language plpgsql;
...
begin
    ...
    select id into budget_team_account_id from public.create_team_account(new.id, 'My First Budget');

    -- Get the budget ID that was created by create_team_account
    select id into new_budget_id 
    from public.budgets b 
    where b.team_account_id = budget_team_account_id;
...
create or replace function public.create_team_account(primary_owner_user_id uuid, account_name text) 
...
begin
    ...
    -- Create the team account (let trigger handle slug)
    insert into public.accounts(
        primary_owner_user_id,
        name,
        is_personal_account)
    ...
    raise log 'Account created successfully: %', new_account.id;
    ...
    raise log 'Budget created successfully: %', new_budget_id;

    -- Insert hidden category group
    ...
    raise log 'Category group created successfully';
    return new_account;
exception when others then
    raise log 'Error in create_team_account: % %', SQLERRM, SQLSTATE;
    raise;
end;
$$ language plpgsql;
